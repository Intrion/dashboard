...
var measureDefinitionEqual = function (key1, key2) { return key1.displayText() === key2.displayText(); };
...
var ItemColorScheme = (function (_super) {
    ...
    function ItemColorScheme(item, _requestColorSchemeDelegate) {
        var _this = _super.call(this) || this;
        _this._requestColorSchemeDelegate = _requestColorSchemeDelegate;
        ...
        var update = _utils_1.debounce(function () {
            if (!_this.disposed) {
                _this._requestColorSchemeDelegate(_this.componentName).done(function (scheme) {
        ...
        _this.toDispose(item._availableColorSignatures.subscribe(update));
        if (item instanceof data_dashboard_item_1.DataDashboardItem) {
            _this.toDispose(item._subcribeDataQueryParams(update));
        }
        update();
        return _this;
    }
...
var ColorSchemeModel = (function () {
    function ColorSchemeModel(dashboard, requestColorSchemeDelegate) {
        var _this = this;
        this.dashboard = dashboard;
        this.requestColorSchemeDelegate = requestColorSchemeDelegate;
                ...
                items.forEach(function (item) {
                    item._measures.forEach(function (measure) {
                        if (item._isColoringEnabled(measure)) {
                            addMeasureDefinition(_color_scheme_entry_creator_1.ColorSchemeEntryCreator.createMeasureDefinitionFromMeasure(measure));
                        }
                    });
                });
            }
        ...
        this.itemColorSchemes[""] = new ItemColorScheme(dashboard, requestColorSchemeDelegate);
    ...
    ColorSchemeModel.prototype.createItemColorScheme = function (item) {
        this.itemColorSchemes[item.componentName()] = new ItemColorScheme(item, this.requestColorSchemeDelegate);
    };
